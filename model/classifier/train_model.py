...
DATA_PATH  = os.path.join(BASE_DIR, '..', 'data', 'aqm_clean.csv')
...
print("\n[Leakage Analysis] If 'aqi' has >90% importance, you might have target leakage.")
print("This happens if the 'label' was created by a fixed threshold on AQI.")
print("Consider removing 'aqi' to see if the model can predict quality from only Temp/Hum.")
...
with open(os.path.join(OUTPUT_DIR, 'label_encoder.pkl'), 'wb') as f:
    pickle.dump(le, f)
print("\nLabel encoder saved → model/classifier/output/label_encoder.pkl")

with open(os.path.join(OUTPUT_DIR, 'rf_model.pkl'), 'wb') as f:
    pickle.dump(rf, f)
print("RF model saved     → model/classifier/output/rf_model.pkl")
...
print("Scaler saved → model/classifier/output/scaler.pkl")
...
tflite_path = os.path.join(OUTPUT_DIR, 'aqm_model.tflite')
...
print(f"\n{'=' * 50}")
print(f"Training complete. Files ready for Raspberry Pi:")
print(f"  model/classifier/output/aqm_model.tflite")
print(f"  model/classifier/output/label_encoder.pkl")
print(f"  model/classifier/output/scaler.pkl")
print(f"{'=' * 50}")